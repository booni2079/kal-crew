...
$(document).ready(function(){
	...
	$(window).resize(function(){
		...
		setResizeNav();	//mobile menu(세로탭) 윈도우 사이즈 변환시 위치 변환(renewal) - ADD 2023.01

	});

	$(window).scroll(function(){
		gnbFixed();
		headerFixed();	//header fixed(renewal) - ADD 2023.01
	...
	$('textarea').each(function () {
		if ($(this).parents('.comment-area').length == 0) {
			var style = 'height:' + (this.scrollHeight + 3) + 'px;'
			if (!_.isEmpty(this.getAttribute('maxHeight'))) {
				style = style + 'max-height:' + this.getAttribute('maxHeight')
			}
	...
	/* 통합검색:Start */
	$('.renewal .gnb-wrapper .ico-btn-search').on('click',function() {
		$body.css('overflow','hidden');
		$('body').addClass('scroll-lock');
		$dimmed.show();
		$('.renewal .header').addClass('search-open');
		$('.renewal .search-wrap').slideDown(200);
	});
	$('.renewal .search-wrap .ico-btn-hide').on('click',function() {
		$body.css('overflow','inherit');
		$('body').removeClass('scroll-lock');
		$dimmed.hide();
		$('.renewal .header').removeClass('search-open');
		$('.renewal .search-wrap').hide();
	});
	/* //통합검색:End */

	/* usermenu widget(my info/로그아웃) */
	$('.renewal .usermenu-item ._has-toggle').on('click', function() {
		$(this).next().toggleClass('_show');
		$(".renewal .usermenu-item ._has-toggle").not(this).next().removeClass('_show');
	});
	...
	$('.m-gnb .ico-btn-close').on('click',function(){
		...
		$dimmed.css('z-index',5);
		$dimmed.hide().removeClass('mp_show');
		$('body').css('overflow-y','inherit');
		$('body').removeClass('scroll-lock');
	});
	/* //mGNB:End */

	/*----- ADD 2023.01 : 리뉴얼 디자인 추가 ------End */

});
// end: $(document).ready

/* mobile menu(세로탭) 윈도우 사이즈 변환시 위치 변환(renewal) - ADD 2023.01 */
...
/* header fixed(renewal) - ADD 2023.01 */
